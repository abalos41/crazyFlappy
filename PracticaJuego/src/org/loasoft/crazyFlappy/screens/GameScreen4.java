...
public class GameScreen4 implements Screen, InputProcessor{
	...
	public GameScreen4(crazyFlappy juego) {
		 
		this.juego = juego;
			
		fondo = new Texture(Gdx.files.internal("data/fondo4.jpg"));
		spriteCesped = new Texture(Gdx.files.internal("data/cesped4.jpg"));
		...
		sonidoCuervo = Gdx.audio.newSound(Gdx.files.internal("data/ghost.mp3"));
	...
	private void playCurrentLevelMusic() {
		...
		if (ManagerSound.isSoundEnabled()) {
			sonidoFondo = Gdx.audio.newMusic(Gdx.files.internal("data/dantes.mp3"));
			sonidoFondo.setVolume(0.10f);
	...
	public void actualizar(float dt){
		...
		//Cuando pasa de 100 segundos has ganado
		
		if(tiempoJuego > 100){
			dispose();
			juego.setScreen(new GameScreenFinalWin(juego));
		}
	...
	public void generarEnemigos(){
		...
		crow = new Crow(new Vector2(Constants.SCREEN_WIDTH, y), velocidadBichos,
				new Texture(Gdx.files.internal("data/ghost.png")));
