...
public class GameScreen3 implements Screen, InputProcessor{
	...
	public GameScreen3(crazyFlappy juego) {
		 
		this.juego = juego;
			
		fondo = new Texture(Gdx.files.internal("data/fondo3.jpg"));
		spriteCesped = new Texture(Gdx.files.internal("data/cesped3.png"));
		...
		sonidoCuervo = Gdx.audio.newSound(Gdx.files.internal("data/stoneimpact.mp3"));
	...
	private void playCurrentLevelMusic() {
		...
		if (ManagerSound.isSoundEnabled()) {
			sonidoFondo = Gdx.audio.newMusic(Gdx.files.internal("data/unicornio.mp3"));
	...
	public void actualizar(float dt){
		...
		if(tiempoJuego > 70){
			dispose();
			juego.setScreen(new GameScreen3Win(juego));
		}
	...
	public void generarEnemigos(){
		...
		crow = new Crow(new Vector2(Constants.SCREEN_WIDTH, y), velocidadBichos,
				new Texture(Gdx.files.internal("data/stone3.png")));
